...
@Component
public class DisposableBackTaskDsSaleDataTask implements SchedulingConfigurer {
	
	private static final Logger logger = LogManager.getLogger(DisposableBackTaskDsSaleDataTask.class);
	...
	private static String CRON = "0 30 20 13 5 ?"; // 0秒0分1时 每日每月不管星期几
	...
	private void autoStatisticDsSaleData() throws BaseException {
		...
		List<User> users;
		try {
			filter.getRules().add(new SearchRule("jobType", Constants.ROP_CN, "sales"));
			...
			String dayFirst = sdf.format(DateUtil.getThisYearFirst());
			for (User user : users) {
				...
				String customerCount = "SELECT count(*),ossuserid from erp_customer"
						 + " where ossuserid =" + "\"" +  user.getOssUserId() + "\""
						 + " and wtime >= " + "\"" + dayFirst + "\""
						 + " group by ossuserid";
				//查询新增客户数
				List<Object[]> customerAddList = (List<Object[]>) baseDao.selectSQL(customerCount);
				...
				dsOrderFilter.getRules().add(new SearchRule("wtime", Constants.ROP_GE, DateUtil.getThisYearFirst()));
		...
		try {
			...
			for (User user : users) {
				String sql = "select sum(salesmoney) as totalmoney,customername,ossusername" + " from erp_ds_order"
						+ " where ossuserid =" + "\"" + user.getOssUserId() + "\"" + " and wtime >= " + "\"" + dayFirst
