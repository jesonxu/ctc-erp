...
@Service("billPaymentFlowService")
public class BillPaymentFlowService extends BaseFlowTask {
	private static Logger logger = LogManager.getLogger(BillPaymentFlowService.class);
	public static final String FLOW_CLASS = Constants.BILL_PAYMENT_FLOW_CLASS;
	public static final String FLOW_NAME = Constants.BILL_PAYMENT_FLOW_NAME;

	private static final String UPDATE_PRODUCTBILL_SQL = "update erp_bill set actualpayables = actualpayables+? where id =?";
	...
	public String verifyFlowMsg(ErpFlow erpFlow, String productId, String labelValue) {
		...
		return convertJsonToCharge(labelValue, erpFlow.getFlowId(), null) + verifyPayablesThisables(labelValue);
	}
					...
					if (json != null) {
						...
						if (billArray != null && billArray.size() > 0) {
							for (int i = 0; i < billArray.size(); i++) {
								...
								BigDecimal thisPayment = new BigDecimal(bill.getString(Constants.BILL_THIS_PAYMENT_KEY));
								int result = baseDao.executeSqlUpdte(UPDATE_PRODUCTBILL_SQL, new Object[] { thisPayment, productBillId }, new Type[] {
										StandardBasicTypes.BIG_DECIMAL, StandardBasicTypes.STRING });
								if (result == 0) {
									logger.info("id：" + productBillId + "的产品账单记录增加实付金额失败，增加金额：" + thisPayment);
								}
				...
				if (json != null) {
					...
					if (billArray != null && billArray.size() > 0) {
						for (int i = 0; i < billArray.size(); i++) {
							...
							if (StringUtils.isNotBlank(bill.getString(Constants.BILL_THIS_PAYMENT_KEY))) {
								thisPayment = new BigDecimal(bill.getString(Constants.BILL_THIS_PAYMENT_KEY));
							}
							amountMoney = amountMoney.add(thisPayment);
						}
						// 找到产品在 付款截止日期 所在月的现金流记录，没有就创建
						SearchFilter filter = new SearchFilter();
						String time = json.getString(Constants.PAYMENT_END_TIME_KEY);
						...
						if (cacheFlowList != null && !cacheFlowList.isEmpty()) {
							...
							cashFlow.setEntityType(EntityType.SUPPLIER.ordinal());
						...
						if (auditResult == AuditResult.CREATED.getCode()) {
							...
							// 驳回流程，把付款金额从现金流的应付减去，因为发起人节点可以取消流程，或修改金额重新申请
							if (cashFlow.getPayables().compareTo(amountMoney) >= 0) {
	...
	private String verifyPayablesThisables(String labelValue) {
		try {
			...
			if (json != null) {
				...
				if (billArray != null && billArray.size() > 0) {
					for (int i = 0; i < billArray.size(); i++) {
						JSONObject bill = billArray.getJSONObject(i);
						if (StringUtils.isNumeric(bill.getString("actualpayables")) && StringUtils.isNumeric(bill.getString(Constants.BILL_THIS_PAYMENT_KEY))
								&& StringUtils.isNumeric(bill.getString(Constants.BILL_PAYABLES_KEY))) {
							BigDecimal actualpayables = bill.getBigDecimal("actualpayables");
							BigDecimal thisPayment = bill.getBigDecimal(Constants.BILL_THIS_PAYMENT_KEY);
							BigDecimal payables = bill.getBigDecimal(Constants.BILL_PAYABLES_KEY);
							if (payables.compareTo(actualpayables.add(thisPayment)) < 0) {
								return "第" + (i + 1) + "个账单,已付金额[" + actualpayables + "]加本次付款[" + thisPayment + "]大于应付金额[" + payables + "]";
							}
						}
					}
				}
