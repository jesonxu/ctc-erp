...
$(document).ready(function() {
	layui.use('laydate', function () {
		...
		laydate.render({
			elem: '#firstGenerateBillTime',
			value: $('#firstGenerateBillTime').val(),
			format: 'yyyy-MM-dd',
    ...
    if (productType === 4 || productType === '4') {
        $('.yysTypeDiv').hide();
    }
    initSelect();
    initButton();
...
function getFormData() {
    ...
    var formData =  {
        ...
        firstGenerateBillTime: $("#firstGenerateBillTime").val()||"",
...
function initBillTaskDay() {
    ...
    var day = $("#billTaskDayH").val();
    $("#billTaskDay").find("option[value='"+day+"']").attr("selected",true);
}
...
function loadDirectChannel() {
    ...
    var directConnect = $('#directConnect').val() === 'true';
    $.ajax({
        type: "POST",
        async: false,
        ...
        success: function (data) {
            if (isNotBlank(data) && data.length > 0) {
                $.each(data, function (index, item) {
                    options = options + "<option value='" + item.value + "' " + (directConnect && account === item.value ? "selected" : "") + ">" + item.name + "</option>";
                });
