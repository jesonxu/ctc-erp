...
function init() {
    layui.use(['layer', 'table', 'form', 'element', 'laydate'], function () {
        ...
        loadDsSignData(sale_statistic_year, yearMonth);
    });
}
...
// 初始化签单统计表格
function initDsSignDataTable(id, data, yearMonth) {
        ...
        cols: [
            [{
                ...
                templet: function (res) {
                    return res.CustomerCount ? thousand(res.CustomerCount) : '0';
                },
                style: fontSize,
                totalRow: true,
                ...
                align: 'center',
                templet: function (res) {
                    return res.totalCost ? thousand(parseFloat(res.totalCost).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
                ...
                field: 'oldCustomerCount',
                title: '老客户数',
                align: 'center',
                templet: function (res) {
                    return res.oldCustomerCount ? thousand(res.oldCustomerCount) : '0';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 0, thousand: true}
...
function initChangeYear(id, data, yearMonth) {
    laydate.render({
        ...
        format:"yyyy年MM月",
            ...
            var time = year + "-" + month;
            loadDsSignData(sale_statistic_year, time);
        }
    })
}

// 加载签单统计数据统计
function loadDsSignData(year, yearMonth) {
    var loading = layer.load(2);

    // 查询签单统计的数据
    $.ajaxSettings.async = true;
    $.post("/dsSaleData/getDsSignStatistics", {
        deptId: deptId,
        userId: userId,
        queryDate: yearMonth
        ...
        if (status === "success") {
            if (!isNull(data.data)) {
                // 填充签单统计数据
                initDsSignDataTable('#realRoyalty', data.data, yearMonth);
            }
        }
        layer.close(loading);
    });
