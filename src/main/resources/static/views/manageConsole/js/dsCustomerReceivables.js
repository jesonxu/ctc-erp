...
function init() {
    layui.use(['layer', 'table', 'form', 'element', 'laydate'], function () {
        ...
        loadDsCustomerReceivables(sale_statistic_year, yearMonth);
    });
}
...
function open_filter_tab() {
    ...
    layer.open({
        ...
        yes: function (index, layero) {
            ...
            loadDsCustomerReceivables(year, year);
            layer.close(index);
        }
    });
    setTimeout(function () {
...
// 初始电商客户应收账款统计表
function initDsCustomerReceivablesTable(id, data, yearMonth) {
    ...
    table.render({
        ...
        cols: [
            [{
                ...
                align: 'center',
                templet: function (res) {
                    return res.needReturn ? thousand(parseFloat(res.needReturn).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
                ...
                align: 'center',
                templet: function (res) {
                    return res.realReturn ? thousand(parseFloat(res.realReturn).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
...
function initChangeYear(id, data, yearMonth) {
    laydate.render({
        ...
        format:"yyyy年MM月",
            ...
            var time = year + "-" + month;
            loadDsCustomerReceivables(sale_statistic_year, time);
        }
    })
}

// 加载电商客户应收账款统计表
function loadDsCustomerReceivables(year, yearMonth) {
    var loading = layer.load(2);

    // 查询销售的提成数据
    $.ajaxSettings.async = true;
    $.post("/dsSaleData/getDsSaleReturn.action", {
        deptId: deptId,
        userId: userId,
        queryDate: yearMonth
        ...
        if (status === "success") {
            if (!isNull(data.data)) {
                // 填充提成数据
                initDsCustomerReceivablesTable('#realRoyalty', data.data, yearMonth);
            }
        }
        layer.close(loading);
    });
