...
function init() {
    layui.use(['layer', 'table', 'form', 'element', 'laydate'], function () {
        ...
        loadDsSaleData(sale_statistic_year, yearMonth);
    });
}
...
// 初始化销售数据统计表格
function initDsSaleDataTable(id, data, yearMonth) {
    ...
    table.render({
        ...
        cols: [
            [{
                ...
                align: 'center',
                templet: function (res) {
                    return res.orderNo ? thousand(res.orderNo) : '0';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 0, thousand: true}
                ...
                align: 'center',
                templet: function (res) {
                    return res.orderTotalPrice ? thousand(parseFloat(res.orderTotalPrice).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                ...
                align: 'center',
                templet: function (res) {
                    return res.returnMoney ? thousand(parseFloat(res.returnMoney).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
                ...
                align: 'center',
                templet: function (res) {
                    return res.performanceGoal ? thousand(parseFloat(res.performanceGoal).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
                ...
                align: 'center',
                templet: function (res) {
                    return res.profitGoal ? thousand(parseFloat(res.profitGoal).toFixed(2)) : '0.00';
                },
                style: fontSize,
                totalRow: true,
                totalConfig: {decimal: 2, thousand: true}
            }, {
                field: 'performanceGoalRate',
                title: '业绩完成率',
                align: 'center',
                style: fontSize,
                totalRow: false,
...
function initChangeYear(id, data, yearMonth) {
    laydate.render({
        ...
        format:"yyyy年MM月",
            ...
            var time = year + "-" + month;
            loadDsSaleData(sale_statistic_year, time);
        }
    })
}

// 加载销售数据统计
function loadDsSaleData(year, yearMonth) {
    var loading = layer.load(2);

    // 查询销售的提成数据
    $.ajaxSettings.async = true;
    $.post("/dsSaleData/getDsSaleData.action", {
        deptId: deptId,
        userId: userId,
        queryDate: yearMonth
        ...
        if (status === "success") {
            if (!isNull(data.data)) {
                // 填充提成数据
                initDsSaleDataTable('#realRoyalty', data.data, yearMonth);
            }
        }
        layer.close(loading);
    });
