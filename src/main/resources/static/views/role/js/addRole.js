...
function initTable() {
    layui.use(['table', 'form'], function () {
		...
		table.render({
            id: 'menuTable',
            elem: '#menuTable',
            url: '/role/addRolesData.action?temp=' + Math.random(),
            cols: [[
                {
                    field: 'name',
                    title: "菜单名称",
			...
			, parseData: function (res) {// 数据加载后回调
				...
				$.each(res.data, function (i, item) {
					...
					if (item.status) {
						if (menuTypes[item.consoleType] == null) { // type：2资源，4客户
							menuTypes[item.consoleType] = 1;
						} else {
							menuTypes[item.consoleType] = menuTypes[item.consoleType] + 1;
						}
					}
				});
		...
		form.on('switch(modifyStatus)', function(data){
			...
			table.reload('pagePermission', {
				url: '/role/getPagePermission?type=' + typeStr
			})
		});
		...
		pagePermissionTable = table.render({
			...
			cols: [
				[{
					...
					align: 150,
					templet: function (d) {
						var html = '<input id = ' + d.desc + ' type="checkbox" name="pageCheckbox" lay-skin="switch" lay-filter="pageStatus" lay-text="开启|关闭">';
						return html;
					}
				}]
			], parseData: function (res) {// 数据加载后回调
