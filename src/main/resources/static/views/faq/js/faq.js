...
(function($) {
    ...
    $.fn.goFaq = function(options) {
        ...
        function search (e) {
			var el, container, filter, count, pattern, answers, toc, tocs, empty;
			el = $(this);
		...
		function generateEmptySearch () {
			...
			return $empty.html ('未找到相关内容');
		}
        
        function generateSearchForm () {
        	
        	var $form = $('<form>', { 'class': 'faq-search' });
        	var $input = $('<input>', { 'type': 'text', 'name': 'search', 'placeholder': '请输入关键词', 'class': 'layui-input' });
        ...
        function generateToc ($list) {
			...
			$list.find ('li').each (function (i) {				
				var id = parseInt (i+1);							
				html += '<li>' + id + '. <a href="#faq-' + id + '">' + $(this).find ('h2').text () + '</a></li>';
				$(this).attr ('id', 'faq-' + id);				
			});	
