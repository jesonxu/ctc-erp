...
function init_add_supplier_btn() {
    ...
    $("#add_supplier_info").click(function (e) {
        ...
        layer.open({
            ...
            fixed: false, //不固定
            maxmin: true,
            content: '/supplier/toAddSupperPage'
        });
    });
}
...
function load_supplier_info(supplier_id) {
    $.ajax({
        type: "GET",
        async: false,
        url: "/supplier/readSupplierInfoById/" + supplier_id + ".action?temp=" + Math.random(),
        dataType: "html",
...
function to_edit_supperlier_baseinfo(supplier_id) {
    openDialogIndex = layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: '/supplier/toEditSupperlierBaseinfo/' + supplier_id + "?r=" + Math.random()
    });
}