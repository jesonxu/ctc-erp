...
var lastScroll = 0;
// 结算 一次加载是否完成
var DS_SETTLEMENT_LOAD_COMPLETED = true;
// 运营 一次加载是否完成
var DS_OPERATE_LOAD_COMPLETED = true;
 ...
 * @param callback      回调函数
 */
function loadSupplierSettlementByPage(product_id, year, month, page, pageSize,callback) {
    var url;
    var data;
    ...
    $.post(url, data, function (res, status) {
        if (status == "success") {
            ...
            // 加载流程详情
            loadSettlementDetailByPage(flowEntIds,callback);
 ...
 * @param callback 回调函数
 */
function loadSettlementDetailByPage(ids, callback) {
...
function loadSupplierAllSettlement(supplierid) {
    ...
    $.post("/settlement/getAllSettlement", {"supplierId": supplierId, "entityType": 2}, function (data, status) {
        if (status == "success") {
            var settlement_ele = $("#settlement_depart");
            settlement_ele.html(data);
...
/**
 * 初始化流程发起按钮
 * @param entityType    0供应商，1客户, 2电商供应商
 * @param flowType      0运营，1结算
 * @param productid     点击产品id
 */
function initSettlementFlowButton(entityType, flowType) {
    $.ajax({
        type: "POST",
        async: false,
