...
//	var initInvoiceInfos = initinvoice(false);
    ...
    re_show_files();
    // 上传文件回显
    function re_show_files() {
        // 已经上传文件
        var loaded_files = $("#contractFilesInfo").val();
        var loaded_files1 = $("#financialFileInfo").val();
        var loaded_files2 = $("#manageCertificationFileInfo").val();
        if (!isBlank(loaded_files) && loaded_files.length > 0) {
            // 解析数据
            var contract_file = JSON.parse(loaded_files);
            if (contract_file.length > 0) {
                for (var file_index = 0; file_index < contract_file.length; file_index++) {
                    var file_temp = contract_file[file_index];
                    if (!isBlank(file_temp)) {
                        var file_temp_json = JSON.stringify(file_temp);
                        var index = hex_md5(file_temp.filePath);
                        var tr = $(["<tr id='upload-my-" + index + "'>", "<td>" + file_temp.fileName + "</td>", "<td><span style='color: #5FB878'>已经上传</span></td>", "<td>", "<button type='button' class='layui-btn layui-btn-xs my-down-load' onclick='viewFile(" + file_temp_json + ")'>预览</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-down-load' onclick='down_load(" + file_temp_json + ")'>下载</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-delete' onclick='delete_file(" + file_temp_json + ")'>删除</button>", "</td>", "</tr>"].join(""));
                        sto_index[index] = file_temp;
                        file_infos.push(file_temp);
                        $("#demoList").append(tr);
                    }
                }
                $('#demoList').parent().show();
            }
        }
        if (!isBlank(loaded_files1) && loaded_files1.length > 0) {
            // 解析数据
            var contract_file = JSON.parse(loaded_files1);
            if (contract_file.length > 0) {
                for (var file_index = 0; file_index < contract_file.length; file_index++) {
                    var file_temp = contract_file[file_index];
                    if (!isBlank(file_temp)) {
                        var file_temp_json = JSON.stringify(file_temp);
                        var index = hex_md5(file_temp.filePath);
                        var tr = $(["<tr id='upload-my-" + index + "'>", "<td>" + file_temp.fileName + "</td>", "<td><span style='color: #5FB878'>已经上传</span></td>", "<td>", "<button type='button' class='layui-btn layui-btn-xs my-down-load' onclick='viewFile(" + file_temp_json + ")'>预览</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-down-load' onclick='down_load(" + file_temp_json + ")'>下载</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-delete' onclick='delete_file1(" + file_temp_json + ")'>删除</button>", "</td>", "</tr>"].join(""));
                        sto_index1[index] = file_temp;
                        file_infos1.push(file_temp);
                        $("#demoList1").append(tr);
                    }
                }
                $('#demoList1').parent().show();
            }
        }
        if (!isBlank(loaded_files2) && loaded_files2.length > 0) {
            // 解析数据
            var contract_file = JSON.parse(loaded_files2);
            if (contract_file.length > 0) {
                for (var file_index = 0; file_index < contract_file.length; file_index++) {
                    var file_temp = contract_file[file_index];
                    if (!isBlank(file_temp)) {
                        var file_temp_json = JSON.stringify(file_temp);
                        var index = hex_md5(file_temp.filePath);
                        var tr = $(["<tr id='upload-my-" + index + "'>", "<td>" + file_temp.fileName + "</td>", "<td><span style='color: #5FB878'>已经上传</span></td>", "<td>", "<button type='button' class='layui-btn layui-btn-xs my-down-load' onclick='viewFile(" + file_temp_json + ")'>预览</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-down-load' onclick='down_load(" + file_temp_json + ")'>下载</button>", "<button type='button' class='layui-btn layui-btn-xs layui-btn-danger my-delete' onclick='delete_file2(" + file_temp_json + ")'>删除</button>", "</td>", "</tr>"].join(""));
                        sto_index2[index] = file_temp;
                        file_infos2.push(file_temp);
                        $("#demoList2").append(tr);
                    }
                }
                $('#demoList2').parent().show();
            }
        }
    }
    // 提交
    $("#supplier_submit").click(function (e) {
        ...
        $.ajax({
            type: "POST",
            async: false,
            url: "/supplier/editSupplier.action?entityType=2&temp=" + Math.random(),
            dataType: 'json',
            data: form_data,
            success: function (data) {
                if (data.code == 200) {
...
function delete_file(file_info) {
    ...
    $("#upload-my-" + file_code).remove();
    var file_index = sto_index[file_code];
    var posi = $.inArray(file_index, file_infos);
    // 删除元素
    file_infos.splice(posi, 1);
    console.log(JSON.stringify(file_infos))
}
function delete_file1(file_info) {
    // 获取需要删除的id
    var file_code = hex_md5(file_info.filePath);
    $("#upload-my-" + file_code).remove();
    var file_index = sto_index1[file_code];
    var posi = $.inArray(file_index, file_infos1);
    // 删除元素
    file_infos1.splice(posi, 1);
    console.log(JSON.stringify(file_infos1))
}
function delete_file2(file_info) {
    // 获取需要删除的id
    var file_code = hex_md5(file_info.filePath);
    $("#upload-my-" + file_code).remove();
    var file_index = sto_index2[file_code];
    var posi = $.inArray(file_index, file_infos2);
    // 删除元素
    file_infos2.splice(posi, 1);
    console.log(JSON.stringify(file_infos2))
}
// 下载
...
function getFormData() {
    ...
    var formData = {
        // 必要
        supplierId:$("#supplierId").val(), //公司ID
        ...
        bankInfos: JSON.stringify(bankinfos), //银行信息
        // 非必要
