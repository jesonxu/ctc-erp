...
// 加载供应商所有产品的运营时间
function loadSupplierAllOperate(supplierid) {
    var loadingIndex = layer.load(2);
    productId = '';
    supplierId = supplierid;
    $.ajaxSettings.async = true;
    $.post("/operate/getAllOperate.action", {"supplierId": supplierId}, function (data) {
        if (isNotBlank(data)) {
            $('#priceCard').html(data);
            // 初始化折叠框
            initSupplierOperatePanel();
...
function loadSupplierOperateByPage(date, productid, page, pageSize, callback) {
    if (isNotBlank(date)) {
        var data;
        var url;
        if (isBlank(productId)) { // 查询所有产品的运营记录
 ...
 * @param label_name
 */
function timeAccountBillData(iframe, label_id, label_name) {
    var dataDom = iframe.find(".layui-show").find("#" + label_id);
 ...
 * @param isVerify
 */
function platformAccountInfoData(iframe, labelId, labelName, isVerify) {
    isVerify = isNull(isVerify) ? false : isVerify;
    var dataDom = iframe.find(".layui-show").find("#" + labelId);
    var dataAccountItems = $(dataDom).find("div[data-platform-account-index]");
    if (dataAccountItems.length === 0) {
        return null;
    }
    var data = [];
    for (var index = 0; index < dataAccountItems.length; index++) {
        var item = dataAccountItems[index];
        var itemIndex = $(item).attr("data-platform-account-index");
        var account = $(item).find("input[name='account']").val();
        if (isBlank(account)) {
            if (isVerify) {
                layer.msg(labelName + ':第' + itemIndex + "个账户信息，账户不能");
            }
            return null;
        }
        var password = $(item).find("input[name='password']").val();
...
// 显示各个流程的发起按钮
function initOperateFlowButton(entityType, flowType, productid) {
    $.ajax({
        type: "POST",
        async: false,
            ...
            if (data !== "") {
                ...
                $.each(data, function (i, item) {
                    html += "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary applyFlow' style='margin: 0'" +
                        " data-my-id='" + item.flowId + "'>" + item.flowName + "</button>";
                });
