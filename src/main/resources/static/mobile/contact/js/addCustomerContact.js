layui.use(['layer', 'element', "form", "upload", "laydate"], function () {
     ...
     * 初始化 客户/供应商 关联信息（选择框）
     */
    function initCustomerChoose() {
        // 设置对应名称
        $("#customer-choose").click(function () {
            let entityId = $("#entity-id").val();
            let index = layer.open({
                type: 2,
                area: ['100%', '100%'],
                title: "选择客户",
                content: "/mobile/toEntityChoose?associateType=0&entityId=" + entityId + "&noPublic=1"
            });
            layer.full(index);
        });
    }
    ...
    function bindSubmitBtn() {
        $("#submit").click(function (e) {
            ...
            $.ajax({
                type: "POST",
                async: false,
                url: "/contact/addCustomerContact.action?temp=" + Math.random(),
                data: formData,
