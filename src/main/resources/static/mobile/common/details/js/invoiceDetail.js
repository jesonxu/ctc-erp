layui.use(['jquery', 'layer', 'element','form'], function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.code === 200) {
                    ...
                    let isEmpty = "";
                    if (util.isNull(checkedItem)) {
                        isEmpty = " checked='' "
                    }
                    invoiceEle.append("<li><input type='radio' name='invoice' " + isEmpty + "title='不选择'></li>");
                    let invoices = data.data;
                    for (let index = 0; index < invoices.length; index++) {
                        let invoiceInfo = invoices[index];
                        let value = invoiceInfo.value;
                        let checked = "";
                        if (util.isNotNull(checkedItem) && value.indexOf(checkedItem) >=0) {
                            checked = " checked='' ";
                        }
                        let text = invoiceInfo.text;
                        let title = invoiceInfo.title;
                        title = title.replace(/(\r\n)|(\n)|(\r)/g,'<br>');
                        invoiceEle.append("<li><input type='radio' name='invoice' value='" + value + "' " + checked + " data-invoice-text='" + text + "' title='" + title + "'></li>");
                    }
    ...
    function bindChooseEvent() {
        let invoiceList = $("#invoice-info-list").find("li");
        $(invoiceList).each(function (index, item) {
            $($(item).find("div[class*='layui-form-radio']")[0]).click(function (event) {
                close();
            });
        });
    }
