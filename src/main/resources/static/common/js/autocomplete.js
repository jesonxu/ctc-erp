layui.define(['jquery', 'laytpl', 'layer'], function (e) {
	"use strict";
	layui.link('/common/css/autocomplete.css');
	var hint = layui.hint(),
		$ = layui.jquery,
		laytpl = layui.laytpl,
		layer = layui.layer,
	...
	job.prototype.pullData = function () {
		...
		(!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
			...
			success: function (resp) {
				_hidelem.val('');
				return 0 != eval('resp.' + _config.response.code) ?  layer.msg('未查询到数据') : _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
			},
			error: function () {
				hint.error("请求失败")
			},
	...
	job.prototype.events = function () {
		...
		$(document).on('click', function (e) {
			...
			if (_e !== undefined) {
				if (_e.attr('autocomplete-load') !== undefined)
					return false;
						_elem.val(laytpl(_config.template_name).render(_config.data[_self.index][_e.index()])),
						_config.onselect == undefined|| _config.onselect(_config.data[_self.index][_e.index()])
			}
